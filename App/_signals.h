...
typedef union { //структура сигнала "AO_PC_ERR_MAIN"
	struct {
		unsigned no_bcu			:1; //нет связи с мод. упр. гидротормозом
		unsigned no_fc			:1; //нет связи с преобр. часоты
		unsigned no_ta			:1; //нет связи с сервоприводом педали газа
		unsigned servo_not_init	:1; //калибровка сервопривода
		unsigned no_trq			:1; //нет связи с датчиком момента
		unsigned err_fc			:1; //ошибка в работе преобр. частоты
		unsigned err_akb		:1; //низкое напряжение АКБ.
		unsigned err_starter	:1; //стартер не включился
		unsigned err_brake		:1; //ощибка гидротормогза
		unsigned err_fc_speed	:1; //Электропр. не может достичь зад. обор.
		unsigned engine_start	:1; //ошибка запуска двигателя.
		unsigned engine_rotate	:1; //двиг. не выходит на заданные обороты
		unsigned emergancy_stop	:1; //нажать "Аварийный стоп"
		unsigned no_eacc		:1; //нет связи с эмул-м пед.и газа (MVU6)
		unsigned servo_error	:1; //ошибка сервопривода
		unsigned no_cruise		:1; //ошибка управления по Круиз-Контроль
		unsigned servo_init_ok	:1; //калибровка сервопривода завершена
		unsigned pid_self_tune	:1; //Автонастройка ПИД рег-ра контура скор.
		unsigned pid_stune_ok	:1; //Автонастройка ПИД рег-ра завершена
		unsigned err_stune		:1; //Ошибка автонастройки ПИД регулятора
		unsigned ext_contrl		:1; //Внешнее управление двигателем
		unsigned int_contrl		:1; //Локальное управление двигателем
	} bit;
	uint32_t dword;
} error_t;
