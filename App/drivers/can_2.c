...
static uint8_t can_2_buf_overflow;		  //счётчик переполнений буфера
static uint32_t can_2_buf_read_pointer; //указатель в буфере на данные которые ещё не обработаны
static uint32_t can_2_buf_write_pointer; //указатель в буфере на данные куда будет записан следующий пакет

static uint8_t can_2_fnumber; //номер фильтра для инициализации
static uint8_t can_2_ffifo; //номер FIFO на котором надо инициализировать фильтр
...
void can_2_init(uint8_t prescaler)
{
	...
	can_2_clear_buf(); //очистить приёмный буфер
	CAN_2.pRxMsg=&can_2_buf[can_2_buf_write_pointer]; //установить указатель на приём пакета в первый элемент буфера
	CAN_2.pTxMsg=&can_2_tx;							  //инициализировать переменную для формирования отправляемого пакета
	can_2_ffifo=CAN_FIFO0; can_2_fnumber=CAN_2_START_FILTER;						  //установить указатель на инизиализацию первого фильтра FIFO0
	can_2_set_filter32(0,0, 0,0, CAN_FILTERMODE_IDMASK); //фильтр установить на приём всех пакетов
	can_2_ffifo=CAN_FIFO0; can_2_fnumber=CAN_2_START_FILTER;						 //вернуть указаетль на фильтр 0, т.к. при вызове функции внешним модулем данный фильтр должен быть переинициализирован
...
{CAN_FilterConfTypeDef  CAN_FilterInitStructure;
	...
	if (can_2_fnumber>=CAN_ALL_FILTERS) return(0); //если использованы все свободные фильтры

	CAN_FilterInitStructure.FilterNumber = can_2_fnumber; //Номер фильтра
	...
	if (can_2_ffifo==CAN_FIFO0)	CAN_FilterInitStructure.FilterFIFOAssignment = CAN_FILTER_FIFO0;//фильтр для FIFO0
	...
	if (can_2_ffifo==CAN_FIFO0) //если инициализация фильтра на FIFO0
		{
		HAL_CAN_Receive_IT(&CAN_2, CAN_FIFO0);	//включить приём пакетов на FIFO0
		CAN_2.State = HAL_CAN_STATE_READY;
		can_2_ffifo=CAN_FIFO1;					//следующий фильтр будет установлен на FIFO1
		}
	else
		{
		HAL_CAN_Receive_IT(&CAN_2, CAN_FIFO1);	//включить приём пакетов на FIFO1
		CAN_2.State = HAL_CAN_STATE_READY;
		can_2_ffifo=CAN_FIFO0;					//следующий фильтр будет установлен на FIFO0
		}
	can_2_fnumber++;
...
/**
  ...
  *
  * @retval смещение от 0 до CAN_2_BUF_SIZE-1
  */
...
{uint32_t sz=0, rx_point, pt;
	...
	if (rx_point<can_2_buf_read_pointer) //если указатель приёма переходил через конец буфера
		{
...
uint8_t can_2_write_tx_data(uint32_t id, uint8_t length, uint8_t* data)
{
	...
	if (id>0x7FF)	CAN_2.pTxMsg->IDE = CAN_ID_EXT; //если размер дареса больше 11 бит, то расширенный ID = 29бит
	else			CAN_2.pTxMsg->IDE = CAN_ID_STD; //иначе ID 11бит

	CAN_2.pTxMsg->RTR=0;	//пакеты удалённых запросов не поддерживаются (не рекуомендуется поддержка удалённых запросов, т.к. они аппаратно по разному обрабатываются и поддерживаются на различных устйроствах)
	...
	if (HAL_CAN_Transmit(&CAN_2, CAN2_TX_TIMEOUT)==HAL_OK)	return(1); //если пакет отправлен
	else return(0);
}
