...
modbus_rx_t modbus_rx[MODBUS_MAX_DEV]; //указатели на функции обработчики пакетов от устйроств CanOpen
static uint8_t modbus_channel=0;//рабочий канал RS-485 на котором инициализирована надстройка ModBus
static stime_t modbus_tx_time; //время когда разрешен следующий приём
static stime_t modbus_rx_time; //врея когда должен быть принят следующий байт
static uint8_t modbus_busy;		//состояние интерфейса ModBus: 0-свободен, 1-ожидание пакетв ответа на запрос
static uint16_t modbus_rx_size; //размер принятых данных
static uint8_t modbus_buf[256];
static uint8_t modbus_rq_tx[MODBUS_MAX_DEV]; //таблица запросов отправки на случай если линия занята ожиданием ответа или передачей
static uint8_t modbus_rq_point=0; //указатель в таблице запросов на устйроство с максимальным приоритетом отправки: этот мехнизм нужен чтобы гарантировать всем устйроства на шине модбас равноценный доступ к процедуре отправки
...
{uint32_t rx_size;
 ...
 uint8_t cnt;
 	 ...
 	 if (timers_get_time_left(modbus_tx_time)==0)
 		 modbus_busy=0; //если не дождались ответа, то освободить  интерфейс

#ifdef RS485_1_UART_NUMBER
	if (modbus_channel==1)	//если ModBus на канале 1
		rx_size=rs485_1_get_rx_size();	//прочитать кол-во принятых байт
#endif
#ifdef RS485_2_UART_NUMBER
	if (modbus_channel==2)	//если ModBus на канале 2
		rx_size=rs485_2_get_rx_size();	//прочитать кол-во принятых байт
#endif
	if (rx_size!=modbus_rx_size) //если количество принятых байт увеличилось
		{
		modbus_rx_size=rx_size;	//обновить кол-во принятых байт
		modbus_rx_time=timers_get_finish_time(MODBUS_BYTE_RX_PAUSE);	//установить максимально допустимое время приёма следующего байта
		return;
		}

	if (rx_size!=0)	//если есть принятые байты
		{
		if (timers_get_time_left(modbus_rx_time)==0)	//если истекло время ожидания следующего байта: приём закончен
			{
#ifdef RS485_1_UART_NUMBER
			if (modbus_channel==1)	//если ModBus на канале 1
				rs485_1_read_rx_data(modbus_buf, rx_size);	//прочитать все принятые байты из буфера
#endif
#ifdef RS485_2_UART_NUMBER
			if (modbus_channel==2)	//если ModBus на канале 1
				rs485_2_read_rx_data(modbus_buf, rx_size);	//прочитать все принятые байты из буфера
#endif
			crc_rx=((uint16_t)modbus_buf[rx_size-2]<<8)|modbus_buf[rx_size-1];		//прочитать CRC принятого пакета
			crc_calc=modbus_crc_rtu(modbus_buf, rx_size-2);	//расчитать CRC принятого пакета
			if (crc_rx==crc_calc)							//если CRC cовпадают
				{
				modbus_busy=0; //если пришёл ответ, то освободить  интерфейс
				for(cnt=0; cnt<MODBUS_MAX_DEV; cnt++)		//найти обработчики принятых пакетов
					if (modbus_rx[cnt]!=NULL)
						modbus_rx[cnt](&modbus_buf[3], modbus_buf[2], modbus_buf[0], modbus_buf[1]); //вызвать обработчик принятых пакетов
				}
			}
		}
}
...
{uint8_t cnt;
	if (adr>0) //если проверка доступности линии с учётом приоритетов
		{
		adr--;
		if (modbus_busy==0) //шина доступна
			{
			cnt=modbus_rq_point;		//начинаем ппроверку проиоритетов запроса с устйроства, которое на данном шаге имеет максимальный приоритет
			while ((cnt!=adr)&&(modbus_rq_tx[cnt]==0)) //проерить есть ли в очереди устйроства с большим приоритетом отправки
				{
				cnt++;
			...
			if (cnt==adr) //если нет устройств с большим приоритетом
				...
				{
				modbus_rq_tx[adr]=1; //установить флаг запроса на отправку
